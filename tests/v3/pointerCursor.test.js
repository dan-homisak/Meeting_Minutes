  ...
  const view = {
    ...
    dispatch(transaction) {
      ...
      if (
        ...
        Number.isFinite(transaction.selection.head)
      ) {
...
test("pointer controller maps rendered source attrs and cursor controller moves by line", () => {
  ...
  const activated = pointer.handlePointer(
    ctx.view,
    {
      target,
      preventDefault() {},
...
test("pointer controller prefers coordinate-mapped positions over fragment range starts", () => {
  ...
  const handled = pointer.handlePointer(
    ctx.view,
    {
      target,
      clientX: 120,
...
test("pointer controller maps clicks inside hidden code-fence source lines", () => {
  ...
  const hiddenFenceLineHost = {
    className:
      "cm-line mm-live-v4-source-code-line mm-live-v4-source-code-line-start mm-live-v4-source-code-fence-hidden",
    getAttribute(name) {
      if (name === "data-src-from") {
        return "0";
      }
...
test("cursor controller snaps vertical entry into fenced code to fence-line end", () => {
  ...
  assert.equal(moved, true);
  assert.equal(
    activeState.doc.lineAt(activeState.selection.main.head).number,
    3,
  );
...
test("cursor controller skips hidden marker gaps for task, ordered, bullet, and quote lines", () => {
  ...
  function setCursorByLineColumn(lineNumber, column) {
    ...
    const position = Math.min(
      line.to,
      line.from + Math.max(0, Math.trunc(column)),
    );
    activeState = activeState.update({
      selection: {
        anchor: position,
  ...
  assert.equal(
    cursor.moveCursorHorizontally(cursorView, -1, "ArrowLeft"),
    true,
  );
  assert.equal(
    activeState.selection.main.head,
    activeState.doc.line(1).from + 5,
  );
  ...
  setCursorByLineColumn(1, 5);
  assert.equal(
    cursor.moveCursorHorizontally(cursorView, 1, "ArrowRight"),
    true,
  );
  assert.equal(
    activeState.selection.main.head,
    activeState.doc.line(1).from + 6,
  );
